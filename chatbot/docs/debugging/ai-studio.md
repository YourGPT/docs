...
## 1. Variable Logs
  ...
  1. Open the emulator from the top-right **Emulator** button.

    ![Open the Emulator](/assets/chatbot/studio-2/image-1.png)

  2. From inside the emulator, open **Variable Logs** .
...
> **TIP:** - **Session details (FLOW object)**: in the emulator, you can open session details (Info / “i”) to view session metadata and the `FLOW` object (variables + current stored values). This view may not update live—close and reopen to refresh.
...
## 2. Execution Logs
  ...
  1. Open the Logs drawer using the bottom-right **Logs** toggle.
