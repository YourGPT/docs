  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2  gap-4  ">
      {items.map(({ title, description, icon, link }) => (
        <div key={title} className="card-component p-6 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm dark:shadow-md h-[200px] md:h-[180px] hover:border-gray-400 dark:hover:border-blue-400 transition-all duration-500 !mt-0 bg-white dark:bg-inherit">
          ...
          <a href={link} className="no-underline font-semibold transition-all duration-300 text-lg mt-4 inline-block pl-[35px] text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300">Learn more</a>
        </div>
      ))}
    </div>
  );
