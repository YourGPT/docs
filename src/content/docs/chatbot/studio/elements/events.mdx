...
3. Choose from the available events:
   ...
   - **`post_knowledgebase`**: Triggers every time after the AI replies from the knowledge base.

   - **`marked_as_resolved`**: Triggers every time a user session is marked as resolved.

4. Add the actions or responses in the **`Flow`** section that will be activated when a particular event takes place.


> Learn more about Events in our helpdesk article: [How to use Events in Chatbot Studio](#)