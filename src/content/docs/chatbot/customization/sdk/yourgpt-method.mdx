...
<div className='text-4xl font-extrabold border border-b-1 border-t-0 border-x-0 pb-6 dark:text-white mb-12 !leading-[50px]'> $yourgpt Method </div>
...
To integrate the Chatbot Web SDK into your website, you need to inject the following script:
 ...
 ```js js title="example.js" file="CMS"
 <script 
...
<Aside > 📝 Make sure to load this script before accessing the `$yourgptChatbot` variable for calling functions. The `$yourgptChatbot` variable is only available after the script is loaded.</Aside>
...
<Aside > 📝 This command will open the chatbot if it is not already open.</Aside>
...
<Aside > 📝 For contact update, either `email` or `phone` must be provided.</Aside>
...
To set visitor data, append the `VISITOR_DATA.keyName=value` query parameter to your iframe URL.
 ...
 ``` js file="CMS" title="example.html" 
...
<Aside > 📝  Here {`<widgetUrl>`} is the URL of your chatbot widget.</Aside>
...
<Aside > 📝 You can pass multiple query parameters by separating them with an `&` .</Aside>
...
<div style={{ height: '20px' }}></div>
 ...
 ```js file="CMS" title="example.curl"
